let adventInput = `15-16 l: klfbblslvjclmlnqklvg
...
let stringResult = adventInput.split("\n");
...
for (i = 0; i < stringResult.length; i++) {
	let criteria = stringResult[i].split(": ")[0];
	let password = stringResult[i].split(": ")[1];  
	let criteriaLetter = criteria.split(" ")[1];
  	let criteriaNumbers = criteria.split(" ")[0];
  	let criteriaNumber1 = criteriaNumbers.split("-")[0];
 	let criteriaNumber2 = criteriaNumbers.split("-")[1];
  
  
	//if (password.count(criteriaLetter) >= (criteriaNumber1) && <= (criteriaNumber2)) {
	//	let validPasswords += 1;
	//}
	
	if (password === criteriaLetter) {
		count++;
	}

	if (count >= criteriaNumber1 && count <= criteriaNumber2) {
		validPasswords +=1;
	}
